...
$stmt = $conn->prepare("
    SELECT m.id, m.message, m.sent_at, m.attachment, m.attachment_type, m.is_deleted, m.is_edited,
           u.first_name, u.last_name, u.user_id, m.user_id AS sender_id
...
<div class="fadeIn flex h-[calc(100vh-64px)] bg-white overflow-hidden w-full">
    ...
    <div class="hidden md:flex w-80 lg:w-96 flex-col border-r border-gray-100 flex-shrink-0 bg-[#f0f2f5]/30">
        ...
        <div class="h-16 flex-shrink-0 bg-[#f0f2f5] px-4 flex items-center justify-between border-b border-gray-200/60">
            <div class="flex items-center gap-3">
                <div class="size-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-xs font-black shadow-sm">
                    <?= strtoupper(substr($user->first_name, 0, 1) . substr($user->last_name, 0, 1)) ?>
                </div>
                <h3 class="text-sm font-bold text-gray-800">Chats</h3>
            </div>
            <div class="flex items-center gap-2 text-gray-500">
                <button onclick="openSupportModal()" class="p-2 hover:bg-gray-200/50 rounded-full transition cursor-pointer"
                    title="Support Chat">
    ...
    <!-- ── Chat Header ──────────────────────────────────────── -->
    <div class="h-16 flex-shrink-0 bg-[#f0f2f5] px-4 flex items-center justify-between border-b border-gray-200/60 shadow-sm relative z-[60]">
        ...
        <div id="headerDefaultContent" class="flex items-center justify-between w-full">
            <div class="flex items-center gap-3">
                ...
                <div class="size-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white shadow-sm flex-shrink-0">
                    <i class="bx bx-group text-xl"></i>
                </div>
                <div class="min-w-0">
                    <h2 class="text-sm md:text-base font-bold text-gray-800 truncate"><?= htmlspecialchars($user->class) ?> Class</h2>
                    <div class="flex items-center gap-1.5 overflow-hidden">
                        ...
                        <p class="text-[10px] font-semibold text-gray-500 truncate" id="onlineCount">
                            <?= $classCount ?> members
                            <?php if ($lastSeenTime): ?>
                                <span class="hidden md:inline mx-1 font-black opacity-20">•</span>
        ...
        <div id="headerSearchContent"
            ...
            <div class="px-2 text-[10px] font-black text-gray-400 uppercase tracking-widest" id="searchCount">0 matches
            </div>
        </div>
    </div>
    ...
    <div id="chatMessages" class="flex-1 overflow-y-auto px-4 md:px-12 py-6 space-y-3 scroll-smooth relative z-10 custom-scrollbar">
        ...
        if (count($initial) === 0): ?>
            <div id="emptyState" class="flex flex-col items-center justify-center h-full text-center py-20">
                ...
                <h4 class="text-lg font-black text-gray-700 mb-1">Start the Conversation!</h4>
                <p class="text-sm text-gray-400 font-medium max-w-xs leading-relaxed">Be the first to send a message to your
        ...
        <?php else: ?>
                ...
                if ($msgDate !== $currentDate):
                    ...
                    <?php if (!$isMine): ?>
                        <div
                            class="size-8 shrink-0 rounded-full bg-gradient-to-br from-purple-400 to-blue-500 flex items-center justify-center text-white text-[10px] font-black shadow-sm">
                            <?= $initials ?>
                        </div>
                    <?php endif; ?>
                                                                            <div class="max-w-[85%] md:max-w-[72%] <?= $isMine ? 'items-end' : 'items-start' ?> flex flex-col gap-1 relative group">
                        <?php if (!$isMine): ?>
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-wide px-1"><?= $name ?></p>
                        <?php endif; ?>
                        ...
                        <?php else: ?>
                            <div
                                class="px-4 py-3 rounded-2xl text-sm font-medium leading-relaxed shadow-sm flex flex-col gap-2 relative
                                                                                                                                                        <?= $isMine
                            ...
                            <?php if ($msg->attachment): ?>
                                        <?php if ($msg->attachment_type === 'image'): ?>
                                                        ...
                                                        <img src="/school_app/uploads/chat/<?= $msg->attachment ?>" alt="Attachment" 
                                ...
                                <?php else: ?>
                                    <div
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <a href="/school_app/uploads/chat/<?= $msg->attachment ?>" download
                                                class="text-[10px] font-black underline uppercase tracking-widest hover:opacity-100 opacity-70">
                                                Download File
                                            </a>
                                        </div>
                                    </div>
    ...
    <div class="shrink-0 bg-[#f0f2f5] px-3 md:px-6 py-3 relative z-20">
        <div class="flex items-center gap-2 md:gap-4 max-w-full lg:max-w-7xl mx-auto">
            ...
            <div class="flex items-center gap-1 text-gray-500">
                <button id="emojiToggleBtn" type="button" class="p-2 hover:bg-gray-200/60 rounded-full transition cursor-pointer text-2xl leading-none">
                    😊
                </button>
                <div id="emojiPicker" class="hidden absolute bottom-16 left-4 w-72 bg-white border border-gray-100 rounded-3xl shadow-2xl p-4 z-50">
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Pick an Emoji</p>
                    <div id="emojiGrid" class="grid grid-cols-8 gap-1 max-h-48 overflow-y-auto"></div>
                </div>
                
                <!-- Hidden Attachment Input -->
                <input type="file" id="attachmentInput" class="hidden" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.txt">
                <button onclick="document.getElementById('attachmentInput').click()" class="p-2 hover:bg-gray-200/60 rounded-full transition cursor-pointer">
            ...
            <!-- Input Box -->
            <div class="flex-1 relative">
                <div id="attachmentPreview" class="hidden absolute bottom-full left-0 right-0 bg-white p-3 border border-gray-100 rounded-t-2xl shadow-xl animate-in slide-in-from-bottom-2">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="size-10 rounded-lg bg-blue-50 flex items-center justify-center text-blue-600" id="previewIcon">
                                <i class="bx bx-file text-xl"></i>
                            </div>
                            <div class="min-w-0">
                                <p class="text-xs font-bold text-gray-800 truncate" id="previewName">filename.pdf</p>
                                <p class="text-[10px] font-medium text-gray-400" id="previewSize">2.4 MB</p>
                            </div>
                        </div>
                        <button onclick="clearAttachment()" class="p-1 hover:bg-red-50 hover:text-red-500 rounded-full transition">
...
(function() {
    ...
    const MY_NAME = <?= json_encode($user->first_name . ' ' . $user->last_name) ?>;
    const MY_INITIALS = <?= json_encode(strtoupper(substr($user->first_name, 0, 1) . substr($user->last_name, 0, 1))) ?>;
    ...
    function buildBubble(msg, isMine) {
        const name = escHtml(msg.first_name + ' ' + msg.last_name);
        ...
        return `
            <div class="flex items-end gap-3 ${isMine ? 'flex-row-reverse' : ''} chat-msg animate-in slide-in-from-bottom-2" data-id="${msg.id}">
                ${!isMine ? `<div class="size-8 shrink-0 rounded-full bg-gradient-to-br from-purple-400 to-blue-500 flex items-center justify-center text-white text-[10px] font-black shadow-sm">${initials}</div>` : ''}
                <div class="max-w-[85%] md:max-w-[72%] ${isMine ? 'items-end' : 'items-start'} flex flex-col gap-1 relative group">
                    ${!isMine ? `<p class="text-[10px] font-black text-gray-400 uppercase tracking-wide px-1">${name}</p>` : ''}
                    ${msgBody}
    ...
    window.deleteMsg = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('/school_app/student/auth/delete_message.php', { msg_id: id }, function(res) {
                    if(res.success) {
    ...
    window.editMsgUI = function(id, text) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('/school_app/student/auth/edit_message.php', { msg_id: id, message: result.value }, function(res) {
                    if(res.success) {
    ...
    function sendMessage() {
        ...
        $.ajax({
            url: '/school_app/student/auth/send_message.php',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    function pollMessages() {
        $.ajax({
            url: '/school_app/student/auth/fetch_messages.php',
            type: 'GET',
            data: { 
                last_id: lastId,
                ...
                if (res.success) {
                    ...
                    if (res.messages.length > 0) {
                        ...
                        res.messages.forEach(function(msg) {
                            ...
                            if (statusEl) {
                                statusEl.innerHTML = `<?= $classCount ?> members <span class="hidden md:inline mx-1 font-black opacity-20">•</span> <span class="hidden md:inline">Last message: ${timeStr}</span>`;
                            }
                        });
