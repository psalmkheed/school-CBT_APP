...
<div class="fadeIn w-full p-4 md:p-8 min-h-screen">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-3">
                <div
                    class="size-10 rounded-2xl bg-indigo-100 flex items-center justify-center shrink-0 shadow-sm border border-indigo-200">
                    <i class="bx bx-book-open text-indigo-600 text-2xl"></i>
                </div>
                <div>
                    <h3 class="text-xl md:text-2xl font-black text-gray-800 tracking-tight" id="pgTitle">Study Material
                    </h3>
        ...
        <div class="flex items-center gap-3">
            <button onclick="$('#uploadModal').removeClass('hidden')"
                class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg shadow-indigo-200 transition-all flex items-center gap-2 text-sm">
                <i class="bx bx-cloud-upload text-lg"></i> Upload New
            </button>
        </div>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 mb-6" id="categoryFilters">
        <button onclick="changeTab('All')"
            class="tab-btn px-4 py-2 rounded-full text-xs font-bold transition-all border <?= $tab === 'All' ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-200' : 'bg-white text-gray-500 hover:bg-gray-50 border-gray-200' ?>"
            data-tab="All">All Resources</button>
        <button onclick="changeTab('Assignment')"
            class="tab-btn px-4 py-2 rounded-full text-xs font-bold transition-all border <?= $tab === 'Assignment' ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-200' : 'bg-white text-gray-500 hover:bg-gray-50 border-gray-200' ?>"
            data-tab="Assignment">Assignments</button>
        <button onclick="changeTab('Syllabus')"
            class="tab-btn px-4 py-2 rounded-full text-xs font-bold transition-all border <?= $tab === 'Syllabus' ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-200' : 'bg-white text-gray-500 hover:bg-gray-50 border-gray-200' ?>"
            data-tab="Syllabus">Syllabus</button>
        <button onclick="changeTab('Other Download')"
            class="tab-btn px-4 py-2 rounded-full text-xs font-bold transition-all border <?= $tab === 'Other Download' ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-200' : 'bg-white text-gray-500 hover:bg-gray-50 border-gray-200' ?>"
            data-tab="Other Download">Other Downloads</button>
    </div>
    ...
    <div id="materialEmpty" class="hidden flex flex-col items-center justify-center py-20 text-gray-400">
        <div
            class="size-16 bg-white rounded-2xl flex items-center justify-center mb-4 border border-gray-100 shadow-sm">
...
<!-- Modal: Upload Form -->
<div id="uploadModal"
    class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[999] hidden flex items-center justify-center p-4">
    <div
        class="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl scale-100 transform transition-all flex flex-col max-h-[90vh]">
        <div class="px-6 py-5 flex items-center justify-between border-b border-gray-100 bg-gray-50/50">
            <h3 class="font-bold text-gray-800 text-lg flex items-center gap-2"><i
                    class="bx-arrow-big-up-line text-indigo-600"></i> Upload Material</h3>
            <button onclick="document.getElementById('uploadModal').classList.add('hidden')"
                class="w-8 h-8 rounded-full hover:bg-gray-200 flex items-center justify-center text-gray-500 transition"><i
        ...
        <form id="uploadForm" class="p-6 space-y-4 overflow-y-auto">
            <div>
                ...
                <input type="text" name="title" required placeholder="E.g. Math Revision Notes"
                    class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-semibold text-gray-700 outline-none focus:ring-2 focus:ring-indigo-100 transition-all">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label
                        class="block text-xs font-semibold text-gray-500 uppercase tracking-widest mb-1">Category</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-widest mb-1">Target
                        Class</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-widest mb-1">Description
                    (Optional)</label>
                <textarea name="description" rows="2"
                    class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-semibold text-gray-700 outline-none focus:ring-2 focus:ring-indigo-100 transition-all"></textarea>
            </div>

            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-widest mb-1">Select
                    File</label>
                <div
                    class="relative border-2 border-dashed border-gray-200 bg-gray-50 rounded-xl p-6 text-center hover:border-indigo-400 hover:bg-indigo-50/30 transition-all">
                    <input type="file" name="file" id="fileInput" accept=".jpg,.jpeg,.png,.pdf,.doc,.docx,.mp4,.mp3"
            ...
            <div class="pt-4 mt-6 border-t border-gray-100 flex justify-end gap-3">
                <button type="button" onclick="document.getElementById('uploadModal').classList.add('hidden')"
                    class="px-5 py-2.5 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-100 transition">Cancel</button>
                <button type="submit" id="upBtn"
                    class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold transition shadow-lg shadow-indigo-200 flex items-center gap-2">
...
<div id="previewModal" class="fixed inset-0 bg-black/80 backdrop-blur-md z-[1000] hidden flex flex-col p-4 md:p-8">
    <div class="flex items-center justify-between text-white mb-4">
        ...
        <div class="flex gap-4">
            <a href="#" id="pvDownload" download
                class="w-10 h-10 rounded-full bg-white/10 hover:bg-indigo-600 flex items-center justify-center text-white transition cursor-pointer"
                title="Download directly">
                <i class="bx bx-download text-xl"></i>
            </a>
            <button onclick="closePreview()"
                class="w-10 h-10 rounded-full bg-white/10 hover:bg-rose-600 flex items-center justify-center text-white transition">
                <i class="bx bx-x text-2xl"></i>
            </button>
        </div>
    </div>

    <div class="flex-1 w-full max-w-6xl mx-auto rounded-2xl overflow-hidden bg-gray-900 border border-white/10 flex items-center justify-center relative shadow-2xl"
        id="pvContainer">
...
<script>
    ...
    function renderGrid(data) {
        ...
        data.forEach(m => {
            ...
            if (ext === 'pdf') { icon = 'bx-file-heart text-rose-500'; bg = 'bg-rose-50 border-rose-100'; }
            ...
            else if (['doc', 'docx'].includes(ext)) { icon = 'bxs-file-detail text-blue-600'; bg = 'bg-blue-100 border-blue-200'; }
