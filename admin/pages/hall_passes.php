...
<div class="fadeIn w-full md:p-8 p-4">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-gray-800 tracking-tight">Digital Pass Manager</h1>
                <p class="text-sm text-gray-500 font-medium">Issue Temporary Hall & Gate Passes</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        ?>
            <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm transition-all group relative overflow-hidden">
                ...
                <div class="border-t border-gray-50 pt-4 space-y-2">
                    ...
                    <?php if($isActive): ?>
                        <button onclick="returnPass(<?= $p->id ?>)" class="w-full py-2 bg-<?= $statusColor ?>-50 text-<?= $statusColor ?>-600 font-bold text-xs uppercase tracking-widest rounded-xl hover:bg-<?= $statusColor ?>-100 transition-colors">
                            Mark Returned
                        </button>
                    <?php else: ?>
        ...
        <?php if(empty($passes)): ?>
            <div class="col-span-full py-16 text-center bg-white rounded-3xl border-2 border-dashed border-gray-200">
                ...
                <p class="text-gray-500 font-bold">No active passes issued</p>
            </div>
        <?php endif; ?>
    </div>
</div>
...
<div class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-[9999] flex items-center justify-center p-4 fade-in" id="modalPass">
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden scale-in animate-slideUp">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            ...
            <button class="text-gray-400 hover:text-gray-700 transition" onclick="$('#modalPass').addClass('hidden')"><i class="bx bx-x text-2xl"></i></button>
        </div>
        <form id="formPass" class="p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="destination" placeholder="e.g. Sickbay, Principal's Office, Main Gate" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-rose-400 outline-none text-sm text-gray-700">
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="text-[10px] font-semibold text-gray-500 uppercase tracking-widest block mb-2">Reason</label>
                    <input type="text" name="reason" placeholder="e.g. Feeling unwell" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-rose-400 outline-none text-sm text-gray-700">
                </div>
                <div>
                    <label class="text-[10px] font-semibold text-gray-500 uppercase tracking-widest block mb-2">Duration (Minutes)</label>
                    <input type="number" name="duration" min="5" max="240" value="15" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-rose-400 outline-none text-sm font-bold text-gray-700">
                </div>
            </div>
            
            <button type="submit" class="w-full py-4 bg-rose-600 text-white rounded-xl font-semibold shadow-lg shadow-rose-100 hover:-translate-y-1 hover:shadow-xl hover:bg-rose-700 transition-all mt-6 uppercase tracking-widest text-[10px]">Issue Pass</button>
        </form>
    </div>
</div>
...
<script>
    $('#formPass').on('submit', function(e) {
        ...
        $.post(BASE_URL + 'admin/auth/pass_api.php', $(this).serialize(), function(res) {
    ...
    function returnPass(id){
        Swal.fire({
            ...
            if(result.isConfirmed) {
                $.post(BASE_URL + 'admin/auth/pass_api.php', { action: 'revoke', id: id }, function(res) {
                    if(res.status === 'success') {
                        window.showToast(res.message, 'success');
    ...
    function deletePass(id){
        Swal.fire({
            ...
            text: "This will permanently remove this hall pass from the system.",
            icon: 'warning',
            showCancelButton: true,
            ...
            if(result.isConfirmed) {
                $.post(BASE_URL + 'admin/auth/pass_api.php', { action: 'delete', id: id }, function(res) {
                    if(res.status === 'success') {
                        window.showToast(res.message, 'success');
                        window.loadPage('pages/hall_passes.php');
                    } else {
