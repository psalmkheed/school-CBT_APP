...
/* COUNT TOTAL */
$totalRecords = $conn->query("SELECT COUNT(*) FROM users WHERE role = 'staff'")->fetchColumn();
            ...
            <div class="flex justify-center items-center gap-2">
                ...
                <?php for ($p = 1; $p <= $totalPages; $p++): ?>
                    <?php if ($p == 1 || $p == $totalPages || ($p >= $page - 2 && $p <= $page + 2)): ?>
                        <button
                            class="staff-page-btn w-9 h-9 shrink-0 rounded-xl border <?= $p == $page ? 'border-orange-600 bg-orange-600 text-white shadow-md shadow-orange-100' : 'border-gray-200 bg-white text-gray-500 hover:bg-orange-50 hover:text-orange-600 hover:border-orange-200' ?> flex items-center justify-center text-xs font-bold transition shadow-sm cursor-pointer"
                            data-page="<?= $p ?>">
                            <?= $p ?>
                        </button>
