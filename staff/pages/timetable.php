...
$stmt = $conn->prepare("
    SELECT t.*
    FROM timetables t
    WHERE t.teacher_id = :teacher_id AND t.session_id = :session_id
    ORDER BY 
        FIELD(t.day, 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'), 
        t.start_time ASC
");
$stmt->execute([':teacher_id' => $user->id, ':session_id' => $session_id]);
...
<div class="fadeIn w-full md:p-8 p-4">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div class="flex items-center gap-5 w-full">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-gray-800 tracking-tight">My Teaching Schedule</h1>
                <p class="text-sm text-gray-500 font-medium">Your weekly assigned classes</p>
            </div>
        </div>
    ...
    <?php if (empty($timetable)): ?>
        <div class="flex flex-col items-center justify-center p-12 py-20 bg-white rounded-[2rem] border border-dashed border-gray-200 text-center">
            ...
            <h4 class="text-xl font-bold text-gray-700 mb-2">No Classes Assigned</h4>
            <p class="text-sm text-gray-400 max-w-sm">You have not been assigned any classes in the timetable yet.</p>
        </div>
    <?php else: ?>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50 border-b border-gray-100">
                            ...
                            <th class="p-6 text-xs font-semibold text-gray-400 uppercase tracking-widest">Class</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-gray-50/30 transition-all group timetable-row" data-day="<?= htmlspecialchars($slot['day']) ?>">
                                ...
                                <td class="p-6">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <p class="text-sm font-bold text-gray-600 truncate"><?= htmlspecialchars($slot['class']) ?></p>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
