...
if (empty($questions)) {
    echo '<div class="p-8 flex flex-col items-center justify-center text-center min-h-[60vh]">
        ...
        <button onclick="loadPage(\'/school_app/student/pages/waec_practice.php\')" class="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold text-sm hover:bg-indigo-700 transition-all cursor-pointer">← Back to Subjects</button>
    </div>';
    exit;
}
?>
...
<div class="fixed inset-0 bg-white z-[900] flex flex-col fadeIn" id="practiceQuizContainer">
    ...
    <!-- Header -->
    <div class="h-16 bg-white border-b border-gray-100 px-4 md:px-8 flex items-center justify-between shrink-0 shadow-sm">
        <div class="flex items-center gap-3">
            <button onclick="exitPractice()" class="size-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-red-50 hover:text-red-500 transition-all cursor-pointer" title="Exit Practice">
                <i class="bx bx-x text-2xl"></i>
            </button>
            <div class="hidden md:block">
                <h4 class="text-sm font-bold text-gray-800"><?= htmlspecialchars($subject) ?> Practice</h4>
                <p class="text-xs text-gray-400"><?= $topic ? htmlspecialchars($topic) : 'All Topics' ?></p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <button id="calcToggleBtn" onclick="toggleCalculator()"
                class="size-10 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-400 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-all cursor-pointer shadow-sm"
                title="Calculator">
            ...
            <div class="flex items-center gap-2 bg-indigo-50 px-4 py-2 rounded-2xl border border-indigo-100">
                <i class="bx bx-list-ol text-indigo-600 font-bold"></i>
                <span class="text-sm font-bold text-indigo-600" id="pqProgress">Q1 of <?= count($questions) ?></span>
            </div>
            <!-- Score badge -->
            <div class="flex items-center gap-2 bg-green-50 px-4 py-2 rounded-2xl border border-green-100">
                ...
                <span class="text-sm font-bold text-green-600" id="pqScore">0/0</span>
            </div>
        </div>
    </div>
    ...
    <!-- Question Area -->
    <div class="flex-1 overflow-y-auto bg-gray-50/30 md:p-12 p-5 flex items-start justify-center scroll-smooth" id="pqScrollContainer">
        <div class="max-w-3xl w-full mt-4 md:mt-0 pb-24" id="pqQuestionArea">
            <!-- Injected by JS -->
        </div>
    </div>

    <!-- Footer -->
    <div class="h-20 bg-white border-t border-gray-100 px-6 md:px-12 flex items-center justify-between shrink-0 shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
        <button onclick="pqPrev()" id="pqPrevBtn" disabled
            class="px-6 py-3 bg-gray-100 text-gray-500 rounded-2xl font-bold text-sm hover:bg-gray-200 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2">
            <i class="bx bx-left-arrow-alt text-lg"></i> Prev
        </button>

        <div class="flex gap-1.5 overflow-x-auto max-w-[40%] px-2 scrollbar-none" id="pqDots"></div>

        <button onclick="pqNext()" id="pqNextBtn"
            class="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold text-sm hover:bg-indigo-700 shadow-md transition-all cursor-pointer flex items-center gap-2">
            Next <i class="bx bx-right-arrow-alt text-lg"></i>
        </button>
    </div>
</div>
...
(function() {
    ...
    function render() {
        ...
        let html = `
            <div class="fadeIn">
                <span class="inline-block px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-4
                ...
                <h2 class="text-lg md:text-xl font-bold text-gray-800 mb-8 leading-relaxed">${q.question}</h2>
        ...
        opts.forEach((letter, i) => {
            ...
            if (isRevealed) {
                if (isCorrect) {
                    ...
                    iconHtml = '<i class="bx bx-check-circle text-green-500 text-xl"></i>';
                } else if (isSelected && !isCorrect) {
                    cls = 'border-red-400 bg-red-50';
                    iconHtml = '<i class="bx bx-x-circle text-red-500 text-xl"></i>';
                } else {
            ...
            html += `
                <button onclick="pqSelect(${current}, '${letter}')"
                    class="w-full flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-200 text-left cursor-pointer ${cls}"
                    ${isRevealed ? 'disabled' : ''}>
                    <span class="size-10 shrink-0 rounded-xl ${isSelected && !isRevealed ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600'} flex items-center justify-center font-bold text-sm">${letter}</span>
                    <span class="text-sm font-medium text-gray-700 flex-1">${val}</span>
                    ${iconHtml}
                </button>`;
        });
        ...
        if (isRevealed && q.explanation) {
            html += `
                <div class="mt-6 p-5 rounded-2xl bg-blue-50 border border-blue-100 fadeIn">
                    <div class="flex items-center gap-2 mb-2">
                        <i class="bx bx-bulb text-blue-600 text-lg"></i>
                        <span class="text-xs font-bold text-blue-600 uppercase tracking-wider">Explanation</span>
                    </div>
        ...
        if (userAns && !isRevealed) {
            html += `
                <button onclick="pqReveal(${current})"
                    class="mt-6 w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold text-sm hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all cursor-pointer flex items-center justify-center gap-2">
                    <i class="bx bx-check-double text-lg"></i> Check Answer
                </button>`;
        }
        ...
        document.getElementById('pqQuestionArea').innerHTML = `
            <div class="fadeIn text-center py-8">
                ...
                <div class="flex gap-3 justify-center flex-wrap">
                    <button onclick="window.loadPage('/school_app/student/pages/waec_practice.php')" class="px-6 py-3 bg-gray-100 text-gray-700 rounded-2xl font-bold text-sm hover:bg-gray-200 transition-all cursor-pointer">← Back to Subjects</button>
    ...
    window.exitPractice = function() {
        ...
        window.loadPage('/school_app/student/pages/waec_practice.php');
    };
